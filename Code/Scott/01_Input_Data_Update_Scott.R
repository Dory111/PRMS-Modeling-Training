                            ...
                            end_date)
{
  ...
  # ------------------------------------------------------------------------------------------------
    ...
    if(file.exists(file.path(path_prms_data,"nwis","PRMS","Scott_monitoring_stations.csv")))
    {
      stations <- read_csv(file.path(path_prms_data,"nwis","PRMS","Scott_monitoring_stations.csv")) %>%
        dplyr::pull(Station_ID)
    } else {
                             ...
                             end_date)
{
  ...
  # Get station list if file exists, if it doesnt then read it in from prms data, sort by correct stations
  # write it out, then pull the IDs
  if(file.exists(file.path(path_prms_data,"prism","prism_stations.csv")))
  {
    station_list <- read_csv(file.path(path_prms_data,"prism","prism_stations.csv"))
    ...
    station_list <- read_csv(file.path(path_gsp_data,"inputs","scott_prism_stations.csv")) %>%
      dplyr::rename(StationID = Lookup) %>%
      dplyr::filter(grepl('PRISM',Source)) %>%
      select(StationID,Source,Station,Data_Type,Unit,Start_Date,End_Date,Latitude,Longitude) %>%
      mutate(StationID = Station) %>%
      dplyr::filter(Data_Type == 'Precipitation')
    write.csv(station_list, file.path(path_prms_data,"prism","prism_stations.csv"), row.names = FALSE)
