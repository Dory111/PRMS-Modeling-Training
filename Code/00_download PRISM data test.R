...
path_gsp_data <- 'C:/Users/ChristopherDory/LWA Dropbox/Christopher Dory/Projects/PRMS-Modeling-Training/Data/Input/PRISM/'
...
path_prism_ppt <- paste(path_gsp_data,"Precipitation",sep="")
path_prism_tmin <- paste(path_gsp_data,"Air temperature minimum",sep="")
path_prism_tmax <- paste(path_gsp_data,"Air temperature maximum",sep="")
...
Download_PRISM_Data <- function(download_all)
{
  ...
  # beginning date
  dir_flags = c(dir.exists(paste(path_prms_data,"prism/Precipitation",sep="")),
                dir.exists(paste(path_prms_data,"prism/Air temperature minimum",sep="")),
                dir.exists(paste(path_prms_data,"prism/Air temperature maximum",sep="")))
  ...
  if(download_all == F)
  {
                         ...
                         sep = '')))
    {
      ...
      if(file.exists(paste(path_gsp_data,"last_downloaded_date.RData",sep="")))
      {
        
        load(paste(path_gsp_data,"last_downloaded_date.RData",sep="")) # if the file exists then load it
        start_date <- end_date 
        ...
        start_date[start_date == '--'] <- ymd(paste(y,m,d,sep="-"))
    ...
    } else if(file.exists(paste(path_gsp_data,"last_downloaded_date.RData",sep=""))) {
    ...
    if(flag_date_files == F)
    {
      ...
      # ------------------------------------------------------------------------------------------------
      save(end_date,file=paste(path_gsp_data,"last_downloaded_date.RData",sep=""))
    } else {}
    # ------------------------------------------------------------------------------------------------
  }
  # ------------------------------------------------------------------------------------------------
...
Update_PRISM_Data <- function()
{
  ...
  existing_directories_paths <- list.dirs(path_gsp_data, recursive = FALSE)
  existing_directories_names <- list.dirs(path_gsp_data, full.names = FALSE, recursive = FALSE)
  
  
  
  # ------------------------------------------------------------------------------------------------
  # create output log if it doesnt exist
  if(!(file.exists(paste(path_gsp_data,'PRISM_Update_History.txt',sep = ''))))
  {
    text <- paste(' ##################################\n',
                  'This document details the download history of PRISM data\n',
                  '##################################\n\n')
    cat(text,
        file = paste(path_gsp_data,'PRISM_Update_History.txt',sep = ''))
...
Get_PRISM_Directories <- function()
{
  ...
  prism_data_directories <- list.dirs(path = paste(path_gsp_data, sep = ''), recursive = FALSE)
  ...
  return(list(prism_data_directories,
              paste(path_prms_data,'prism/',sep = '')))
  # ------------------------------------------------------------------------------------------------
}
# ------------------------------------------------------------------------------------------------
...
Create_Dirs_IF_Missing <- function()
{
  ...
  dir.create(paste(path_prms_data,"prism/",sep=""))
  path_prism_ppt=paste(path_prms_data,"prism/Precipitation",sep="")
  dir.create(path_prism_ppt)
  path_prism_tmin=paste(path_prms_data,"prism/Air temperature minimum",sep="")
  dir.create(path_prism_tmin)
  # path_prism_tmean=paste(path_prms_data,"prism/Air temperature mean",sep="")
  # dir.create(path_prism_tmean)
  path_prism_tmax=paste(path_prms_data,"prism/Air temperature maximum",sep="")
