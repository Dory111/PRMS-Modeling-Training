...
get_gsp_data_f <- function(
    ...
    path_inputs, # Path to inputs in shared folder
    path_outputs, # Path to outputs
    ...
    get_nrcs_data=F, # only used in Butte
    start_date="1900-01-01", # Start date that users want
  ...
  if(run_cdec_data==F){}else{
    ...
    allsensors=read.csv(paste(path_inputs,"inputs/list of sensors in CDEC.csv",sep="")) # https://cdec.water.ca.gov/misc/senslist.html
    ...
    area_stations=read.csv(paste(path_inputs,"inputs/",area,"_cdec_stations.csv",sep=""))
    ...
    pb <- winProgressBar(title = "Windows progress bar", # Window title
      ...
      for (i in 1:nrow(sensta)){
        variable_code=sensta[i,"variable_code"]
        variable_name=allsensors[match(variable_code,allsensors$variable_code),"variable_name"]
        unit=allsensors[match(variable_code,allsensors$variable_code),"unit"]
        ...
        if(nrow(df3)==0){}else{ # If data for that sensor
          
          # Create output directory
          dir.create(file.path(path_inputs,"outputs",area,"cdec",variable_name))
          
          # Save results
          write.csv(df3,file.path(path_inputs,"outputs",area,"cdec",variable_name,paste(station_id,durations_name,".csv",sep="")),row.names = F)
        }
  ...
  if(run_nwis_data==F){}else{
    ...
    if (area == "butte") { # For Butte PRMS, the prior code in the else section doesn't work for purposes
      # Get station list
      stations <- read_csv(file.path(path_inputs,"outputs",area,"nwis","PRMS","Butte_monitoring_stations.csv")) %>%
      ...
      for (i in 1:length(stations)) {
        ...
        write.csv(df5,file.path(path_inputs,"outputs",area,"nwis","PRMS",glue::glue("{stations[i]}.csv")),row.names = F) 
      }
      
    } else {
    
      # Get station list
      stations <- read_csv(file.path(path_inputs,"outputs",area,"nwis","nwis_monitoring_stations.csv")) %>%
        dplyr::pull(Station_ID)
      ...
      for (i in 1:length(stations)) {
        ...
        write.csv(df5,file.path(path_inputs,"outputs",area,"nwis","Daily_Mean_Discharge_CFS",glue::glue("{stations[i]}.csv")),row.names = F) 
      }
    } 
  }
  ...
  if (run_cimis_data==T){
    # cimis_key <- read_lines(file=paste0(path_inputs,"inputs/CIMIS_API_key.txt"))
    ...
    huc8_basin_dir=paste0(path_outputs,"inputs/QGIS preprocessing/inputs/HU8 CA watersheds/")
  ...
  if (exists("selected_cimis_stations")==F){} else{
    if(nrow(selected_cimis_stations)==0){
      ...
      # Create output directory
      dir.create(file.path(path_inputs,"outputs"))
      dir.create(file.path(path_inputs,"outputs",area))
      dir.create(file.path(path_inputs,"outputs",area,"cimis"))
      ...
      for (year in years_seq){
        #dir.create(file.path(path_inputs,"outputs",area,"cimis",year))
        ...
        if(is_key_set()) {
          ...
          cimis_data=cimis_data(targets,start.date = start_date_cimis,end.date=end_date_cimis,
        ...
        if (nrow(cimis_data)==0){} else { # if data table is empty or not for the calendar year
          count = count + 1 # Start saving data to the master table - advance count every time
      ...
      write.csv(ppt_master,file.path(path_inputs,"outputs",area,"cimis","ppt.csv"),row.names = F)
      write.csv(tmin_master,file.path(path_inputs,"outputs",area,"cimis","tmin.csv"),row.names = F)
      write.csv(tmax_master,file.path(path_inputs,"outputs",area,"cimis","tmax.csv"),row.names = F)
      write.csv(sorad_master,file.path(path_inputs,"outputs",area,"cimis","sorad.csv"),row.names = F)
      write.csv(eto_master,file.path(path_inputs,"outputs",area,"cimis","eto.csv"),row.names = F)
      ...
      write.csv(cimis_download_data_summary,file.path(path_inputs,"outputs",area,"cimis/cimis_download_data_summary.csv"),row.names = F)
    }
  }
  ...
  noaa_token <- read_lines(file=paste0(path_inputs,"inputs/NOAA_CDO_token.txt"))
  ...
  if(nrow(selected_noaa_stations)==0){
    ...
    # Download data
      ...
      if (nrow(noaa_data)==0){} else {
        ...
        write.csv(ppt,file.path(path_inputs,"outputs",area,"noaa","ppt.csv"),row.names = F)
        write.csv(tmin,file.path(path_inputs,"outputs",area,"noaa","tmin.csv"),row.names = F)
        write.csv(tmax,file.path(path_inputs,"outputs",area,"noaa","tmax.csv"),row.names = F)
      }
    ...
    write.csv(noaa_download_data_summary,file.path(path_inputs,"outputs",area,"noaa","noaa_download_data_summary.csv"),row.names = F)
  }}
  ...
  if (get_nrcs_data==T & area=="butte"){
    ...
    # Get station list
    stations <- read_csv(file.path(path_inputs,"outputs",area,"nrcs","Butte_monitoring_stations.csv")) %>%
    ...
    write.csv(ppt,file.path(path_inputs,"outputs",area,"nrcs","prcp.csv"),row.names = F)
    write.csv(tmin,file.path(path_inputs,"outputs",area,"nrcs","tmin.csv"),row.names = F)
    write.csv(tmax,file.path(path_inputs,"outputs",area,"nrcs","tmax.csv"),row.names = F)
    write.csv(wteq,file.path(path_inputs,"outputs",area,"nrcs","wteq.csv"),row.names = F)
    
  } else {}
  
  ##### Combine CDEC, NWIS, CIMIS, NOAA data #####
  flag_cdec=file.exists(file.path(path_inputs,"outputs",area,"cdec/cdec_download_data_summary.csv"))
  flag_nwis=file.exists(file.path(path_inputs,"outputs",area,"nwis/nwis_download_data_summary.csv"))
  flag_cimis=file.exists(file.path(path_inputs,"outputs",area,"cimis/cimis_download_data_summary.csv"))
  flag_noaa=file.exists(file.path(path_inputs,"outputs",area,"noaa/noaa_download_data_summary.csv"))

  if(flag_cdec==T&flag_nwis==T&flag_cimis==T&flag_noaa==T){
    cdec_download_data_summary=read.csv(file.path(path_inputs,"outputs",area,"cdec/cdec_download_data_summary.csv"))
    nwis_download_data_summary=read.csv(file.path(path_inputs,"outputs",area,"nwis/nwis_download_data_summary.csv"))
    cimis_download_data_summary=read.csv(file.path(path_inputs,"outputs",area,"cimis/cimis_download_data_summary.csv"))
    noaa_download_data_summary=read.csv(file.path(path_inputs,"outputs",area,"noaa/noaa_download_data_summary.csv"))
    ...
    # write.csv(download_data_summary,file.path(path_inputs,"outputs",area,"download_data_summary.csv"),row.names = F)
  } else {}
  ...
  if(get_prism_data==T){
    ...
    # Station list
    station_list=read.csv(file.path(path_inputs,"outputs",area,"prism","prism_stations.csv"))
    ...
    path_prism = paste0(path_inputs,"inputs/")
    ...
    #save(ppt_stack,file=paste0(path_inputs,"outputs/",area,"/prism/","ppt_stack.RData"))
    ...
    for (m in 1:length(ppt_station_list_l)){
      ...
      write.csv(df7,file.path(path_inputs,"outputs",area,"/prism/Precipitation/",paste(names(ppt_station_list_l)[m],".csv",sep="")),row.names = F)
    }
    ...
    # if(update_prism_data==F|add_current_stations==T){
      ...
      #save(tmin_stack,file=paste0(path_inputs,"outputs/",area,"/prism/","tmin_stack.RData"))
    ...
    for (n in 1:length(tmin_station_list_l)){
      ...
      write.csv(df8,file.path(path_inputs,"outputs",area,"prism/Air temperature minimum",paste(names(tmin_station_list_l)[n],".csv",sep="")),row.names = F)
    }
...
#    if(update_prism_data==F|add_current_stations==T){
      ...
      #save(tmax_stack,file=paste0(path_inputs,"outputs/",area,"/prism/","tmax_stack.RData"))
    ...
    for (p in 1:length(tmax_station_list_l)){
      ...
      write.csv(df10,file.path(path_inputs,"outputs",area,"prism/Air temperature maximum",paste(names(tmax_station_list_l)[p],".csv",sep="")),row.names = F)
    }
